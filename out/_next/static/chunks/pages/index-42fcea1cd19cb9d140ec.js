_N_E = (window.webpackJsonp_N_E = window.webpackJsonp_N_E || []).push([
    [7],
    {
        '/0+H': function (e, t, n) {
            'use strict';
            (t.__esModule = !0),
                (t.isInAmpMode = r),
                (t.useAmp = function () {
                    return r(c.default.useContext(l.AmpStateContext));
                });
            var a,
                c = (a = n('q1tI')) && a.__esModule ? a : { default: a },
                l = n('lwAK');
            function r() {
                var e = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {},
                    t = e.ampFirst,
                    n = void 0 !== t && t,
                    a = e.hybrid,
                    c = void 0 !== a && a,
                    l = e.hasQuery,
                    r = void 0 !== l && l;
                return n || (c && r);
            }
        },
        '/EDR': function (e, t, n) {
            (window.__NEXT_P = window.__NEXT_P || []).push([
                '/',
                function () {
                    return n('23aj');
                },
            ]);
        },
        '23aj': function (e, t, n) {
            'use strict';
            n.r(t);
            var a = n('q1tI'),
                c = n.n(a),
                l = n('YFqc'),
                r = n.n(l),
                i = c.a.createElement;
            var o = function () {
                    return i(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '113', height: '70', fill: 'none', viewBox: '0 0 113 70' },
                        i('path', {
                            fill: '#DC3545',
                            d: 'M1.16 37.982c-.099.033-.164 0-.197-.098a.571.571 0 01-.098-.147c-.065-.098-.081-.18-.049-.245.392-2.32 1.225-4.377 2.5-6.174 1.273-1.83 2.89-3.43 4.85-4.802 1.895-1.372 4.05-2.613 6.468-3.724a63.622 63.622 0 017.546-2.989 122.72 122.72 0 005.537-1.862 63.388 63.388 0 005.44-2.352l.98-3.626c.359-1.209.767-2.385 1.224-3.528a22.872 22.872 0 011.715-3.332 14.044 14.044 0 012.45-2.989c.229-.229.49-.457.784-.686.327-.261.67-.474 1.03-.637.391-.163.8-.261 1.224-.294.458-.033.948.049 1.47.245.62.229 1.078.523 1.372.882.294.36.474.751.54 1.176 0 .327-.017.653-.05.98a6.284 6.284 0 01-.147.833c-.36 1.241-.8 2.32-1.323 3.234a12.775 12.775 0 01-1.715 2.401c-.62.686-1.34 1.323-2.156 1.911a31.177 31.177 0 01-2.646 1.666 150.987 150.987 0 00-2.009 7.154 102.078 102.078 0 01-1.96 6.762 45.13 45.13 0 01-2.646 6.37c-1.012 2.025-2.335 3.985-3.969 5.88a7.809 7.809 0 01-1.715 1.519 9.123 9.123 0 01-2.205 1.029 7.903 7.903 0 01-2.499.49 7.883 7.883 0 01-2.597-.245c-.98-.261-1.813-.67-2.499-1.225a9.368 9.368 0 01-1.715-1.911 8.97 8.97 0 01-1.029-2.205c-.228-.784-.343-1.486-.343-2.107 0-1.176.147-2.368.441-3.577a14.35 14.35 0 011.421-3.43 12.401 12.401 0 012.352-2.842 8.455 8.455 0 013.185-1.862c.392-.13.866-.196 1.421-.196.523 0 .997.082 1.421.245.458.13.817.376 1.078.735.164.327.18.719.05 1.176-.131.425-.376.67-.736.735-.326.065-.588.065-.784 0-.196-.065-.31-.261-.343-.588 0-.261.05-.49.147-.686.098-.196.115-.376.05-.539-1.046-.261-1.993-.212-2.843.147a6.529 6.529 0 00-2.205 1.47 9.8 9.8 0 00-1.617 2.156c-.457.817-.85 1.584-1.176 2.303-.36.85-.62 1.78-.784 2.793-.13.98-.13 1.944 0 2.891.196 1.209.62 2.27 1.274 3.185.654.882 1.6 1.535 2.842 1.96 1.438.457 2.777.506 4.018.147 1.242-.392 2.205-1.013 2.891-1.862 1.144-1.405 2.107-3.022 2.891-4.851a51.835 51.835 0 002.058-5.88 119.027 119.027 0 001.617-6.321c.49-2.156 1.013-4.247 1.568-6.272a45.53 45.53 0 01-4.753 1.96l-4.704 1.666c-1.568.523-3.12 1.078-4.655 1.666a39.18 39.18 0 00-4.508 1.96 28.257 28.257 0 00-4.508 2.597 22.698 22.698 0 00-3.92 3.283 17.345 17.345 0 00-2.89 3.92A12.168 12.168 0 001.6 37.688a.332.332 0 00-.098.245c-.032.098-.147.114-.343.049zM41.877 2.555a7.76 7.76 0 00-1.568 2.254c-.36.85-.67 1.731-.93 2.646a70.06 70.06 0 00-.638 2.793 21.631 21.631 0 01-.735 2.695 11.86 11.86 0 002.99-2.156c.881-.85 1.698-1.862 2.45-3.038.195-.327.408-.751.636-1.274.262-.555.474-1.11.637-1.666a6.08 6.08 0 00.245-1.617c0-.49-.13-.882-.392-1.176-.196-.261-.555-.36-1.078-.294-.522.033-1.061.31-1.617.833z',
                        }),
                        i('path', {
                            fill: '#000',
                            d: 'M54.03 2.065c-.23 2.679-.818 5.178-1.765 7.497a50.143 50.143 0 01-3.332 6.615 134.218 134.218 0 01-3.969 6.223 57.517 57.517 0 00-3.675 6.223c.392-.229.768-.49 1.127-.784.36-.294.719-.555 1.078-.784.392-.229.8-.408 1.225-.539.425-.13.915-.147 1.47-.049.49.229.866.539 1.127.931.261.36.425.768.49 1.225.098.457.13.931.098 1.421-.033.457-.065.898-.098 1.323-.163.915-.36 1.862-.588 2.842-.229.947-.457 1.878-.686 2.793-.196.915-.36 1.797-.49 2.646-.13.817-.196 1.568-.196 2.254.065.065.13.098.196.098.098-.033.18-.065.245-.098a29.003 29.003 0 002.597-5.243c.719-1.83 1.454-3.61 2.205-5.341.13-.196.212-.343.245-.441.065-.13.229-.196.49-.196.229.196.376.31.441.343.065.033.033.147-.098.343a40.052 40.052 0 00-1.715 3.871 60.43 60.43 0 01-1.617 4.018c-.555 1.241-1.143 2.287-1.764 3.136-.62.85-1.29 1.274-2.009 1.274a2.94 2.94 0 01-1.813-.588c-.49-.36-.702-.817-.637-1.372.065-1.11.196-2.14.392-3.087.229-.98.474-1.944.735-2.891.261-.947.523-1.927.784-2.94.294-1.013.54-2.14.735-3.381 0-.098.016-.261.05-.49a2.084 2.084 0 000-.637c-.034-.229-.083-.392-.148-.49-.065-.13-.196-.147-.392-.049a5.561 5.561 0 00-1.764 1.176c-.49.49-1.013.996-1.568 1.519a14.619 14.619 0 00-1.764 2.94 78.492 78.492 0 00-1.323 3.332c-.392 1.143-.8 2.27-1.225 3.381a19.881 19.881 0 01-1.519 3.136c-.229.163-.44.229-.637.196-.229 0-.44-.033-.637-.098-.523-.13-.85-.523-.98-1.176.065 0 .065-.033 0-.098-.065-.033-.082-.065-.049-.098a468.836 468.836 0 002.891-8.967c.882-2.973 1.797-5.88 2.744-8.722.98-2.875 2.01-5.684 3.087-8.428a89.566 89.566 0 013.724-8.036 16.44 16.44 0 011.225-1.862 16.882 16.882 0 011.568-1.813 13.928 13.928 0 011.862-1.568 7.81 7.81 0 012.01-1.029c.424-.163.8-.147 1.126.049.36.196.523.36.49.49zm-1.226.245c-.261-.033-.572.13-.93.49-.328.327-.67.719-1.03 1.176-.36.457-.686.915-.98 1.372-.294.425-.49.719-.588.882a35.813 35.813 0 00-1.96 4.214 71.551 71.551 0 00-1.519 4.459 190.65 190.65 0 00-1.274 4.263 74.767 74.767 0 01-1.029 3.528 62.142 62.142 0 003.283-4.655 48.225 48.225 0 002.891-4.998 44.615 44.615 0 002.254-5.194 25.587 25.587 0 001.372-5.145c.065-.13.033-.229-.098-.294-.13-.098-.261-.13-.392-.098zm7.912 28.567c.228 0 .343.049.343.147 0 .065-.017.18-.05.343a196.23 196.23 0 01-1.665 4.263 35.58 35.58 0 01-1.715 3.675c-.588 1.045-1.193 1.895-1.813 2.548-.588.653-1.209 1.013-1.862 1.078-.752.065-1.438-.065-2.058-.392a4.667 4.667 0 01-1.617-1.274 7.147 7.147 0 01-1.078-1.96 6.822 6.822 0 01-.392-2.254c0-1.47.261-2.94.784-4.41.522-1.47 1.176-2.793 1.96-3.969.261-.36.588-.719.98-1.078.424-.36.865-.637 1.323-.833.457-.229.93-.36 1.42-.392.49-.033.948.098 1.373.392.294.261.522.702.686 1.323.065.457.065.931 0 1.421-.066.49-.18.915-.343 1.274-.49 1.34-1.193 2.483-2.107 3.43a40.212 40.212 0 01-2.793 2.548v1.225c.032.425.081.866.147 1.323.065.719.261 1.34.588 1.862.359.49.882.653 1.568.49.522-.13 1.029-.555 1.519-1.274.522-.751 1.012-1.633 1.47-2.646a48.04 48.04 0 001.372-3.234c.424-1.143.816-2.189 1.176-3.136 0-.163.098-.294.294-.392.196-.13.359-.163.49-.098zm-4.851-3.234c-.098-.098-.343.13-.735.686-.36.555-.735 1.258-1.127 2.107a21.847 21.847 0 00-1.03 2.548c-.293.882-.44 1.584-.44 2.107.424-.163.93-.572 1.519-1.225a9.68 9.68 0 001.47-2.156c.424-.784.702-1.552.833-2.303.13-.784-.033-1.372-.49-1.764zM72.677 5.985c.424-.327.849-.49 1.274-.49.392 0 .751.13 1.078.392.359.261.522.67.49 1.225-.033 1.503-.441 3.185-1.225 5.047-.784 1.83-1.78 3.724-2.99 5.684a76.561 76.561 0 01-3.92 5.88 191.644 191.644 0 01-4.066 5.39 79.433 79.433 0 01-.686 2.548c-.229.882-.441 1.78-.637 2.695a27.2 27.2 0 00-.441 2.695 10.648 10.648 0 000 2.499c.065.36.13.702.196 1.029.065.327.18.67.343 1.029.196.327.392.49.588.49.228 0 .424-.114.588-.343.424-.49.865-1.16 1.323-2.009.49-.882.963-1.813 1.42-2.793.458-1.013.899-2.009 1.324-2.989a43.655 43.655 0 011.176-2.597c.032-.098.098-.245.196-.441s.277-.245.539-.147c.196.065.294.163.294.294.032.13.016.229-.05.294-.326.555-.669 1.258-1.028 2.107-.327.817-.686 1.682-1.078 2.597s-.8 1.813-1.225 2.695a13.392 13.392 0 01-1.323 2.352c-.458.686-.948 1.209-1.47 1.568-.523.392-1.078.539-1.666.441a4.068 4.068 0 01-1.96-1.029c-.556-.49-.931-1.192-1.127-2.107-.327-1.437-.409-2.94-.245-4.508.13-1.568.408-3.152.833-4.753.392-1.633.898-3.25 1.519-4.851.62-1.633 1.257-3.234 1.91-4.802 1.242-2.973 2.647-5.733 4.215-8.281a29.152 29.152 0 015.83-6.811zm2.009 1.764a1.82 1.82 0 00.049-.441 2.75 2.75 0 00-.05-.539.458.458 0 00-.244-.343c-.098-.098-.245-.13-.441-.098-.327-.033-.605.147-.833.539-.229.392-.425.719-.588.98a78.717 78.717 0 00-4.9 9.212c-1.34 3.038-2.614 6.337-3.822 9.898a147.155 147.155 0 003.185-4.312 69.986 69.986 0 003.038-4.606c.947-1.6 1.813-3.25 2.597-4.949a40.018 40.018 0 002.009-5.341zm6.891-1.764c.425-.327.85-.49 1.274-.49.392 0 .751.13 1.078.392.36.261.523.67.49 1.225-.033 1.503-.441 3.185-1.225 5.047-.784 1.83-1.78 3.724-2.989 5.684a76.561 76.561 0 01-3.92 5.88 191.644 191.644 0 01-4.067 5.39 79.433 79.433 0 01-.686 2.548c-.229.882-.441 1.78-.637 2.695a27.2 27.2 0 00-.441 2.695 10.648 10.648 0 000 2.499c.065.36.13.702.196 1.029.065.327.18.67.343 1.029.196.327.392.49.588.49.229 0 .425-.114.588-.343.425-.49.866-1.16 1.323-2.009.49-.882.964-1.813 1.421-2.793.457-1.013.898-2.009 1.323-2.989a43.655 43.655 0 011.176-2.597c.033-.098.098-.245.196-.441s.278-.245.539-.147c.196.065.294.163.294.294.033.13.016.229-.049.294-.327.555-.67 1.258-1.029 2.107-.327.817-.686 1.682-1.078 2.597s-.8 1.813-1.225 2.695a13.392 13.392 0 01-1.323 2.352c-.457.686-.947 1.209-1.47 1.568-.523.392-1.078.539-1.666.441a4.068 4.068 0 01-1.96-1.029c-.555-.49-.931-1.192-1.127-2.107-.327-1.437-.408-2.94-.245-4.508.13-1.568.408-3.152.833-4.753.392-1.633.898-3.25 1.519-4.851.62-1.633 1.258-3.234 1.911-4.802 1.241-2.973 2.646-5.733 4.214-8.281a29.152 29.152 0 015.831-6.811zm2.009 1.764a1.82 1.82 0 00.049-.441 2.75 2.75 0 00-.049-.539.458.458 0 00-.245-.343c-.098-.098-.245-.13-.441-.098-.327-.033-.604.147-.833.539-.229.392-.425.719-.588.98a78.717 78.717 0 00-4.9 9.212c-1.34 3.038-2.613 6.337-3.822 9.898a147.155 147.155 0 003.185-4.312 69.986 69.986 0 003.038-4.606c.947-1.6 1.813-3.25 2.597-4.949a40.018 40.018 0 002.009-5.341zm1.128 23.03c-.229 0-.36.033-.392.098 0 .065-.082.229-.245.49a66.662 66.662 0 01-2.01 4.802 61.682 61.682 0 01-2.302 4.459c-.066.098-.18.261-.343.49-.13.196-.294.376-.49.539a1.375 1.375 0 01-.54.343c-.195.033-.358-.082-.49-.343.23-1.764.49-3.152.785-4.165.326-1.013.653-1.927.98-2.744.36-.85.702-1.715 1.029-2.597.36-.915.67-2.14.93-3.675.033-.36.033-.735 0-1.127 0-.425-.31-.719-.93-.882-.294-.065-.686-.098-1.176-.098-.49-.033-.817.049-.98.245a116.23 116.23 0 00-.294 1.862c-.066.62-.294 1.356-.686 2.205a67.332 67.332 0 01-.833 2.352 22.48 22.48 0 00-.833 2.352c-.262.784-.458 1.6-.588 2.45-.13.85-.147 1.748-.05 2.695.066.686.311 1.29.736 1.813.392.555.833.98 1.323 1.274.228.098.457.13.686.098.228-.033.44-.114.637-.245.196-.13.375-.278.539-.441.196-.13.36-.261.49-.392a30.519 30.519 0 003.087-5.488 152.48 152.48 0 002.352-5.782.805.805 0 00-.05-.245.592.592 0 00-.342-.343zm-3.97-10.192c.164.13.328.36.49.686.164.327.213.588.148.784-.066.85-.278 1.372-.637 1.568-.327.163-.752.18-1.274.049-.294-.065-.523-.31-.686-.735a2.356 2.356 0 01.049-1.421c.163-.36.408-.67.735-.931.36-.261.751-.261 1.176 0zm18.08 10.78c.033 0 .05-.033.05-.098l.098-.294a.74.74 0 01.245-.294c.098-.065.228-.065.392 0 .13.065.212.163.245.294.065.13.065.229 0 .294a534.746 534.746 0 00-1.764 4.165 65.478 65.478 0 01-1.862 4.165 55.947 55.947 0 01-2.156 4.165 34.515 34.515 0 01-2.695 4.018c-.164.163-.376.31-.637.441-.262.163-.523.212-.784.147-.523-.163-.915-.441-1.176-.833a3.137 3.137 0 01-.49-1.372c-.098-.817-.066-1.535.098-2.156.196-1.405.522-2.989.98-4.753.49-1.797.963-3.479 1.42-5.047.49-1.6.899-2.956 1.226-4.067.326-1.11.424-1.699.294-1.764-.556-.229-1.046-.245-1.47-.049a3.04 3.04 0 00-1.078.784c-.327.36-.621.768-.882 1.225-.229.457-.458.85-.686 1.176a15.386 15.386 0 00-1.421 2.597 34.236 34.236 0 00-1.078 2.744l-.98 2.842a28.628 28.628 0 01-1.03 2.793c-.163.065-.359.147-.587.245-.229.098-.523.114-.882.049a3.903 3.903 0 01-.98-.588c-.36-.261-.54-.588-.54-.98.621-1.34 1.144-2.842 1.569-4.508.392-1.666.767-3.169 1.127-4.508.098-.425.147-.833.147-1.225V29.75c.032-.392.065-.751.098-1.078.065-.36.18-.686.343-.98.522-.13 1.16-.196 1.91-.196.589.392.932.996 1.03 1.813.032.261.049.523.049.784.032.229.065.474.098.735.294-.261.571-.604.833-1.029.294-.425.604-.833.93-1.225a4.63 4.63 0 011.177-1.078c.457-.327 1.012-.506 1.666-.539 1.012-.065 1.747.147 2.205.637.457.49.718 1.11.784 1.862.065.457.065.931 0 1.421-.033.49-.082.964-.147 1.421-.066.457-.147.882-.245 1.274-.066.36-.115.637-.147.833-.164.947-.36 1.862-.588 2.744a77.668 77.668 0 01-.637 2.646c-.196.882-.409 1.78-.637 2.695a25.758 25.758 0 00-.54 2.842c.72-.98 1.422-2.09 2.108-3.332a82.433 82.433 0 001.96-3.773 90.661 90.661 0 001.715-3.724c.522-1.209.963-2.254 1.323-3.136zm13.087 0c.163-.261.343-.49.539-.686.196-.196.375-.13.539.196a.796.796 0 00-.049.245c.032.065.016.147-.049.245a51.249 51.249 0 01-3.479 6.076c-1.307 1.927-2.973 3.577-4.998 4.949a92.586 92.586 0 00-1.862 4.802 463.142 463.142 0 00-1.862 5.292 489.598 489.598 0 01-1.862 5.292 56.298 56.298 0 01-1.96 4.802c-.719 1.503-1.454 2.81-2.205 3.92-.784 1.143-1.601 1.993-2.45 2.548-.36.261-.735.457-1.127.588a3.655 3.655 0 01-1.323.245c-.556 0-1.046-.13-1.47-.392-.556-.294-.964-.686-1.225-1.176a3.952 3.952 0 01-.54-1.617 2.344 2.344 0 01-.048-.833c0-.294.016-.588.049-.882a17.306 17.306 0 012.303-6.517 33.22 33.22 0 014.116-5.635l3.675-3.577a211.321 211.321 0 003.822-3.822c.13-.653.294-1.274.49-1.862.196-.588.359-1.192.49-1.813-.131.163-.392.31-.784.441-.36.13-.768.196-1.225.196-.327 0-.654-.049-.98-.147a2.154 2.154 0 01-.882-.392c-.36-.392-.67-.85-.931-1.372a4.866 4.866 0 01-.49-1.617c-.196-1.372-.017-2.793.539-4.263.522-1.47 1.16-2.728 1.91-3.773.425-.62.915-1.274 1.471-1.96a13.834 13.834 0 011.862-1.813 8.13 8.13 0 012.156-1.274 4.16 4.16 0 012.352-.196c.392.098.735.343 1.029.735.326.392.49.768.49 1.127.359.065.669.049.931-.049.261-.13.522-.114.784.049.098.36.114.653.049.882a190.166 190.166 0 01-2.058 6.027 108.378 108.378 0 01-2.45 6.076c1.502-1.143 2.744-2.466 3.724-3.969a60.284 60.284 0 002.989-5.096zM92.948 64.981a84.96 84.96 0 003.773-8.624 91.205 91.205 0 002.793-9.065 221.847 221.847 0 01-3.97 4.067 48.047 48.047 0 00-3.87 4.361 27.113 27.113 0 00-2.94 4.753 11.764 11.764 0 00-1.225 5.243 4.61 4.61 0 00.343 1.96c.228.686.653 1.045 1.274 1.078.326 0 .67-.114 1.029-.343.359-.196.718-.49 1.078-.882.326-.36.637-.768.93-1.225.295-.425.556-.866.785-1.323zm11.711-30.625c.098-.196.245-.572.441-1.127.228-.555.457-1.16.686-1.813.261-.686.49-1.356.686-2.009.196-.686.294-1.241.294-1.666.065-.62-.049-1.013-.343-1.176-.262-.163-.654-.13-1.176.098-.425.196-.833.523-1.225.98-.36.425-.719.931-1.078 1.519a18.68 18.68 0 00-.931 1.813c-.294.62-.556 1.209-.784 1.764a93.276 93.276 0 01-1.421 3.332c-.425 1.013-.67 2.14-.735 3.381 0 .392.163.735.49 1.029.359.294.718.278 1.078-.049.62-.392 1.11-.768 1.47-1.127.359-.392.669-.817.931-1.274.261-.457.506-.98.735-1.568.228-.588.522-1.29.882-2.107z',
                        }),
                    );
                },
                s = a.createElement,
                u = function () {
                    return s(
                        'div',
                        null,
                        s(
                            'div',
                            {
                                className:
                                    'flex flex-col items-center justify-center p-2 pt-4 space-y-2 md:mx-auto md:w-4/5 md:justify-between md:flex-row',
                            },
                            s(r.a, { href: '/' }, s(o, null)),
                            s(
                                'div',
                                null,
                                s(
                                    'ul',
                                    { className: 'flex space-x-4 font-semibold md:text-lg md:space-x-12' },
                                    s('li', { className: 'duration-300 hover:text-punch-500' }, s(r.a, { href: '/' }, 'Home')),
                                    s('li', { className: 'duration-300 hover:text-punch-500' }, s(r.a, { href: '/' }, 'About')),
                                    s('li', { className: 'duration-300 hover:text-punch-500' }, s(r.a, { href: '/' }, 'Blog')),
                                    s(
                                        'li',
                                        { className: 'duration-300 hover:text-punch-500' },
                                        s('a', { href: 'https://github.com/shelling94', target: '__blank' }, 'Repository'),
                                    ),
                                    s('li', { className: 'duration-300 hover:text-punch-500' }, s(r.a, { href: '/' }, 'Contact')),
                                ),
                            ),
                        ),
                    );
                },
                h = c.a.createElement;
            var d = function () {
                    return h(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '48', height: '48', fill: 'none', viewBox: '0 0 48 48' },
                        h('path', {
                            fill: '#000',
                            d: 'M19.316 19.018c0-2.264-1.54-2.264-1.54-2.264h-6.338v4.858h5.945c1.027 0 1.933-.329 1.933-2.594zM17.776 24.968h-6.338v5.819h6.046c.91-.026 2.588-.311 2.588-2.829 0-3.02-2.296-2.99-2.296-2.99zM34.002 21.611c-3.364 0-3.832 3.357-3.832 3.357h7.152s.043-3.357-3.32-3.357z',
                        }),
                        h('path', {
                            fill: '#000',
                            d: 'M24 0C10.745 0 0 10.746 0 24s10.745 24 24 24 24-10.746 24-24S37.255 0 24 0zm5.233 14.166h8.983v2.68h-8.983v-2.68zm-4.54 14.062c0 6.646-6.917 6.427-6.917 6.427H6.446V12.883h11.33c3.443 0 6.161 1.901 6.161 5.797s-3.322 4.143-3.322 4.143c4.38 0 4.077 5.405 4.077 5.405zm9.351 3.434c3.62 0 3.491-2.342 3.491-2.342h3.832c0 6.216-7.45 5.79-7.45 5.79-8.94 0-8.366-8.323-8.366-8.323s-.007-8.364 8.365-8.364c8.813 0 7.579 9.45 7.579 9.45H30.213c0 4.046 3.831 3.79 3.831 3.79z',
                        }),
                    );
                },
                m = c.a.createElement;
            var f = function () {
                    return m(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '48', height: '48', fill: 'none', viewBox: '0 0 48 48' },
                        m('path', {
                            fill: '#000',
                            d: 'M44.78 12.54c-2.146-3.677-5.057-6.588-8.734-8.734C32.37 1.66 28.354.587 24 .587c-4.355 0-8.37 1.073-12.047 3.219-3.677 2.146-6.588 5.057-8.734 8.734C1.073 16.217 0 20.233 0 24.586c0 5.23 1.526 9.932 4.578 14.11 3.052 4.176 6.995 7.067 11.828 8.67.562.105.979.032 1.25-.218.27-.25.406-.562.406-.937l-.016-1.687c-.01-1.063-.016-1.99-.016-2.781l-.719.124c-.458.084-1.036.12-1.734.11a13.225 13.225 0 01-2.172-.219 4.855 4.855 0 01-2.093-.937A3.965 3.965 0 019.937 38.9l-.313-.72c-.208-.478-.536-1.01-.984-1.593-.448-.584-.9-.979-1.36-1.187l-.218-.157a2.297 2.297 0 01-.406-.375 1.715 1.715 0 01-.281-.438c-.063-.146-.01-.266.156-.36.167-.094.469-.14.906-.14l.625.094c.417.084.932.333 1.547.75.614.416 1.12.958 1.515 1.624.48.855 1.057 1.506 1.735 1.954.677.448 1.359.671 2.046.671.688 0 1.281-.052 1.782-.155.5-.105.968-.261 1.406-.47.187-1.396.698-2.469 1.53-3.218a21.399 21.399 0 01-3.202-.563c-.948-.25-1.927-.656-2.937-1.22-1.011-.561-1.85-1.26-2.516-2.093-.667-.833-1.214-1.927-1.64-3.28-.427-1.355-.641-2.918-.641-4.688 0-2.521.823-4.667 2.469-6.438-.771-1.895-.698-4.02.218-6.374.605-.188 1.5-.047 2.688.421 1.188.47 2.057.87 2.61 1.204.552.333.994.615 1.328.843a22.187 22.187 0 016-.812c2.062 0 4.062.27 6 .812l1.187-.75c.812-.5 1.771-.958 2.875-1.375 1.104-.416 1.948-.53 2.532-.343.937 2.354 1.02 4.479.25 6.374 1.645 1.771 2.468 3.917 2.468 6.438 0 1.77-.214 3.338-.64 4.703-.427 1.365-.979 2.458-1.656 3.281-.677.823-1.521 1.516-2.531 2.078-1.01.563-1.99.969-2.938 1.219-.948.25-2.016.438-3.203.563 1.083.937 1.624 2.416 1.624 4.437v6.593c0 .375.13.688.391.937.26.25.672.323 1.235.219 4.833-1.604 8.776-4.495 11.828-8.672C46.473 34.517 48 29.814 48 24.585c-.001-4.353-1.075-8.368-3.22-12.045z',
                        }),
                    );
                },
                p = c.a.createElement;
            var v = function () {
                    return p(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '48', height: '48', fill: 'none', viewBox: '0 0 48 48' },
                        p('path', {
                            fill: '#000',
                            d: 'M24 0C10.747 0 0 10.747 0 24s10.747 24 24 24 24-10.747 24-24S37.253 0 24 0zm-6.974 36.281H11.18V18.696h5.845v17.585zm-2.922-19.986h-.039c-1.961 0-3.23-1.35-3.23-3.038 0-1.725 1.308-3.038 3.307-3.038 2 0 3.23 1.313 3.268 3.038 0 1.688-1.268 3.038-3.306 3.038zM38.102 36.28h-5.844v-9.407c0-2.365-.847-3.977-2.962-3.977-1.614 0-2.576 1.088-2.998 2.138-.155.375-.193.9-.193 1.426v9.82h-5.844s.076-15.935 0-17.585h5.844v2.49c.777-1.198 2.167-2.903 5.268-2.903 3.845 0 6.73 2.514 6.73 7.915v10.083z',
                        }),
                    );
                },
                g = a.createElement,
                w = function () {
                    return g(
                        'div',
                        { className: 'p-4 md:p-0 bg-papaya-whip-500 md:py-4' },
                        g(
                            'div',
                            { className: 'mx-auto md:w-4/5' },
                            g('h2', { className: 'text-xl font-semibold text-center text-wine-berry-500 md:text-4xl' }, 'Welcome Here!'),
                            g(
                                'div',
                                { className: 'flex flex-col space-y-4 md:space-x-16 md:flex-row md:justify-center' },
                                g('img', { className: 'object-cover md:h-52 ', src: '/images/banner.jpeg', alt: 'Lam tam nhu' }),
                                g(
                                    'div',
                                    { className: 'space-y-4' },
                                    g(
                                        'h1',
                                        { className: 'text-2xl font-semibold text-center md:text-4xl text-punch-500 md:text-left' },
                                        'L\xe2m T\xe2m Nh\u01b0',
                                    ),
                                    g(
                                        'p',
                                        { className: 'max-w-2xl opacity-70' },
                                        "I\u2019m a Front-end web developer and UI designer. At FPT University, I am pursuing a degree in Software Engineering. I'm also proficient in English, design, and architectural development, as seen by several school assignment projects.",
                                    ),
                                    g(
                                        'div',
                                        { className: 'text-center md:text-left' },
                                        g('button', { className: 'px-4 py-2 font-semibold text-gray-100 rounded-xl bg-punch-500' }, 'Get In Touch'),
                                    ),
                                ),
                                g(
                                    'div',
                                    { className: 'hidden space-y-4 lg:block' },
                                    g(
                                        'a',
                                        {
                                            href: 'https://www.linkedin.com/in/shelling-dahlia/',
                                            className: 'block',
                                            target: '__blank',
                                        },
                                        g(v, null),
                                    ),
                                    g('a', { href: 'https://github.com/shelling94', target: '__blank', className: 'block' }, g(f, null)),
                                    g('a', { href: 'https://www.behance.net/tmnhlm1', className: 'block', target: '__blank' }, g(d, null)),
                                ),
                            ),
                        ),
                    );
                },
                x = n('Yh1s'),
                y = {
                    home: {
                        link: '/',
                        header: {
                            title: 'L\xe2m T\xe2m Nh\u01b0 | Portfolio',
                            canonical: '/',
                            description: 'My Personal Portfolio',
                            isIndexPage: !0,
                        },
                    },
                    404: { link: '/404', header: { title: '', canonical: '/404', description: 'Page Not Found' } },
                },
                b = a.createElement,
                M = function () {
                    return b(
                        'div',
                        { className: 'flex justify-center space-x-16 md:py-8' },
                        b(
                            'div',
                            { className: 'px-2 py-4 space-y-2' },
                            b('h1', { className: 'text-lg font-semibold text-punch-500 md:text-4xl' }, 'About Me'),
                            b(
                                'p',
                                { className: 'max-w-lg opacity-70' },
                                'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequat duis enim velit mollit. Exercitation veniam consequat sunt nostrud amet.',
                            ),
                            b(
                                'div',
                                { className: 'flex flex-col space-y-2 md:space-y-0 md:space-x-8 md:flex-row' },
                                b(
                                    'ul',
                                    { className: 'space-y-2 md:space-y-4' },
                                    b(
                                        'li',
                                        { className: 'flex items-center gap-1' },
                                        b('img', { src: '/images/location.png', alt: '', width: '24', height: '24' }),
                                        ' Location: Ho Chi City, Vietnam',
                                    ),
                                    b(
                                        'li',
                                        { className: 'flex items-center gap-1' },
                                        b('img', { src: '/images/uni.png', alt: '', width: '24', height: '24' }),
                                        ' University: FPT University',
                                    ),
                                    b(
                                        'li',
                                        { className: 'flex items-center gap-1' },
                                        b('img', { src: '/images/phone.png', alt: '', width: '24', height: '24' }),
                                        ' Phone: 0901951454',
                                    ),
                                    b(
                                        'li',
                                        { className: 'flex items-center gap-1' },
                                        b('img', { src: '/images/level.png', alt: '', width: '24', height: '24' }),
                                        ' Level: Fresher',
                                    ),
                                ),
                                b(
                                    'ul',
                                    { className: 'space-y-2 md:space-y-4' },
                                    b(
                                        'li',
                                        { className: 'flex items-center gap-1' },
                                        b('img', { src: '/images/address.png', alt: '', width: '24', height: '24' }),
                                        ' Address: Binh Thanh District',
                                    ),
                                    b(
                                        'li',
                                        { className: 'flex items-center gap-1' },
                                        b('img', { src: '/images/degree.png', alt: '', width: '24', height: '24' }),
                                        ' Degree: Undergraduate',
                                    ),
                                    b(
                                        'li',
                                        { className: 'flex items-center gap-1' },
                                        b('img', { src: '/images/email.png', alt: '', width: '24', height: '24' }),
                                        ' Email: tamnhu.dahlia@gmail.com',
                                    ),
                                    b(
                                        'li',
                                        { className: 'flex items-center gap-1' },
                                        b('img', { src: '/images/major.png', alt: '', width: '24', height: '24' }),
                                        ' Major: Front-end Developer, UI designer',
                                    ),
                                ),
                            ),
                        ),
                        b(
                            'div',
                            { className: 'relative hidden m-8 h-96 cus-border lg:block' },
                            b('img', { src: '/images/image.png', alt: 'lam tam nhu', className: 'z-10 h-full ' }),
                        ),
                    );
                },
                z = c.a.createElement;
            var N = function () {
                    return z(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '96', height: '96', fill: 'none', viewBox: '0 0 96 96' },
                        z('path', { fill: '#fff', d: 'M0 0H96V96H0z' }),
                        z('path', {
                            fill: '#DC3545',
                            fillRule: 'evenodd',
                            d: 'M33.108 6.686c.196 0 .581.282.581.554v1.768h-6.436V7.24c-.097-.272.199-.554.492-.554h5.363zm-.392 30.593l-2.343 5.373-2.144-5.373h4.486zM27.648 4c-1.754 0-3.22 1.387-3.22 3.145v2.696c-.678.551-.973 1.384-.973 2.218v22.066c0 1.207.581 2.227 1.564 2.686l3.021 7.234c.386.926 1.36 1.487 2.333 1.487 1.073 0 1.957-.561 2.342-1.487l3.021-7.234c.976-.552 1.565-1.48 1.565-2.686v-6.12c0-.646-.589-1.293-1.369-1.293-.678 0-1.269.646-1.269 1.293v6.12c0 .281-.286.646-.678.646h-7.313c-.197 0-.58-.273-.58-.646V12.059c0-.273.287-.647.58-.647h7.313c.295 0 .678.282.678.646v10.295c0 .646.591 1.3 1.27 1.3.78 0 1.368-.654 1.368-1.3V12.058c0-.833-.392-1.666-.974-2.217V7.145C36.327 5.387 34.862 4 33.011 4h-5.363zM14.275 6.903l2.653 6.526h-5.224l2.57-6.526zm-1.193 8.878v45.067H9.964V16.512c0-.364.37-.731.64-.731h2.478zm4.855.184c.37 0 .732.273.732.64V60.94H15.55V15.965h2.386zm.824 47.329v4.355c-.092.45-.37.632-.824.632h-7.334c-.27 0-.64-.365-.64-.633v-4.354h8.798zM14.275 4c-1.009 0-1.925.632-2.294 1.632l-3.303 8.337c-.823.64-1.278 1.546-1.278 2.636V67.74c-.092 1.721 1.37 3.168 3.203 3.168h7.334c1.832 0 3.21-1.447 3.21-3.168V16.605c0-1-.554-1.997-1.285-2.636l-3.303-8.337C16.291 4.725 15.283 4 14.275 4zM33.12 52.408c.35 0 .7.351.7.7v1.299h-6.615v-1.3c0-.348.175-.699.607-.699h5.307zm1.132 4.517c.263 0 .614.263.614.611v10.251c0 .254-.263.605-.614.605h-7.654c-.263 0-.614-.351-.614-.605v-10.25c0-.35.263-.612.614-.612h7.654zm-6.44-6.771a3.025 3.025 0 00-3.049 3.041v1.992c-.782.526-1.214 1.394-1.214 2.35v10.338c-.087 1.647 1.302 3.034 3.049 3.034h7.654a3.024 3.024 0 003.049-3.034V57.536c0-.955-.439-1.823-1.221-2.35v-1.99c0-1.743-1.39-3.042-3.041-3.042h-5.227zM84.966 7.962c1.523 0 2.75 1.175 2.75 2.622v53.75c0 1.438-1.227 2.613-2.75 2.613H54.041V7.962h30.925zM51.283 6.615v61.689h-1.897V6.614h1.897zM49.292 4c-.949 0-1.994.538-2.273 1.35h-4.08c-1.895 0-3.324 1.445-3.324 3.157v13.348c0 .627.663 1.174 1.332 1.174.757 0 1.322-.547 1.322-1.174V8.507c0-.362.383-.628.67-.628h3.699v58.886h-3.7c-.286 0-.669-.266-.669-.628V27.264c0-.72-.565-1.258-1.322-1.258-.67 0-1.332.538-1.332 1.258v38.873c0 1.892 1.611 3.422 3.323 3.422h4.08c.471.73 1.325 1.357 2.274 1.357h2.086c.948 0 1.897-.627 2.28-1.357h31.308c2.942 0 5.412-2.247 5.412-5.224V10.584c0-2.887-2.375-5.234-5.412-5.234H53.658C53.187 4.538 52.326 4 51.378 4h-2.086z',
                            clipRule: 'evenodd',
                        }),
                        z('path', {
                            fill: '#DC3545',
                            fillRule: 'evenodd',
                            d: 'M75.49 61.692c-.724 0-1.26.508-1.26 1.108 0 .678.536 1.186 1.26 1.186h4.397c.625 0 1.26-.508 1.26-1.186 0-.6-.635-1.108-1.26-1.108H75.49zM79.95 57.07c-.594 0-1.11.58-1.11 1.154 0 .655.516 1.154 1.11 1.154.68 0 1.197-.499 1.197-1.154 0-.575-.517-1.154-1.198-1.154zM8.212 73.233c-1.841 0-3.212 1.548-3.212 3.4v11.644c0 1.948 1.463 3.4 3.212 3.4h61.803c.646 0 1.193-.578 1.193-1.362 0-.677-.547-1.353-1.193-1.353H8.212c-.369 0-.738-.293-.738-.685V76.632c0-.392.37-.782.738-.782h4.768v2.137c0 .683.64 1.262 1.278 1.262.739 0 1.286-.579 1.286-1.262V75.85h3.575v2.137c0 .683.553 1.262 1.193 1.262.731 0 1.287-.579 1.287-1.262V75.85h3.573v2.137c0 .683.556 1.262 1.287 1.262.64 0 1.193-.579 1.193-1.262V75.85h3.575v2.137c0 .683.547 1.262 1.285 1.262.64 0 1.279-.579 1.279-1.262V75.85h3.489v2.137c0 .683.64 1.262 1.279 1.262.738 0 1.285-.579 1.285-1.262V75.85h3.581v2.137c0 .683.547 1.262 1.187 1.262.74 0 1.287-.579 1.287-1.262V75.85h3.572v2.137c0 .683.557 1.262 1.288 1.262.64 0 1.193-.579 1.193-1.262V75.85h3.575v2.137a1.27 1.27 0 001.285 1.262c.64 0 1.286-.579 1.286-1.262V75.85h3.482v2.137c0 .683.64 1.262 1.285 1.262.732 0 1.279-.579 1.279-1.262V75.85h3.581v2.137c0 .683.547 1.262 1.187 1.262.74 0 1.287-.579 1.287-1.262V75.85h3.573v2.137c0 .683.556 1.262 1.287 1.262.646 0 1.193-.579 1.193-1.262V75.85h3.575v2.137a1.27 1.27 0 001.285 1.262c.64 0 1.286-.579 1.286-1.262V75.85h4.767c.363 0 .732.39.732.782v11.645c0 .392-.37.685-.732.685H75.791c-.64 0-1.186.676-1.186 1.353 0 .784.546 1.361 1.186 1.361h11.374c1.835 0 3.213-1.548 3.213-3.399V76.632c-.185-1.85-1.565-3.399-3.397-3.399H8.212z',
                            clipRule: 'evenodd',
                        }),
                    );
                },
                H = c.a.createElement;
            var C = function () {
                    return H(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '96', height: '96', fill: 'none', viewBox: '0 0 96 96' },
                        H('path', { fill: '#fff', d: 'M0 0H96V96H0z' }),
                        H('path', {
                            fill: '#DC3545',
                            fillRule: 'evenodd',
                            d: 'M64.2 11.78c-.694 0-1.294.604-1.294 1.375 0 .682.6 1.28 1.293 1.28h5.596c.775 0 1.288.518 1.288 1.286v64.297c0 .77-.513 1.282-1.288 1.282H21.078c-.781 0-1.295-.511-1.295-1.282v-5.14c0-.77-.513-1.28-1.293-1.28-.775 0-1.288.51-1.288 1.28v5.14c0 2.136 1.72 3.763 3.789 3.763h1.286v3.257c0 2.134 1.72 3.767 3.789 3.767h48.718a3.754 3.754 0 003.787-3.767V85.5c0-.777-.6-1.288-1.293-1.288-.773 0-1.287.511-1.287 1.288v1.54c0 .767-.608 1.279-1.295 1.279H25.892c-.687 0-1.286-.512-1.286-1.28v-3.257h45.103a3.759 3.759 0 003.789-3.763V21.544h1.286c.687 0 1.293.598 1.293 1.28v57.273c0 .518.602 1.115 1.287 1.115.774 0 1.295-.597 1.295-1.28v-57.28c0-2.135-1.721-3.768-3.79-3.768h-1.292v-3.337c0-2.142-1.721-3.767-3.782-3.767h-5.596zM35.11 5.742c.434 0 .878.439.878.882V8.39h-5.095V6.624c0-.443.443-.882.878-.882h3.339zm4.475 5.204c.613 0 1.32.525 1.32 1.326v5.203H26.153v-5.203c-.09-.623.62-1.326 1.23-1.326h12.202zM31.771 3c-1.844 0-3.51 1.504-3.51 3.536V8.3h-.966c-1.578 0-2.898.882-3.508 2.202h-2.722c-2.197 0-3.863 1.771-3.863 3.883v55.249c.09.882.612 1.41 1.4 1.41.797 0 1.322-.528 1.322-1.32v-55.25c0-.792.522-1.318 1.319-1.318h2.279v4.41c0 1.325 1.141 2.557 2.542 2.557h14.928c1.313 0 2.545-1.15 2.545-2.557v-4.41H59.77c.796 0 1.321-.534 1.321-1.328 0-.793-.525-1.326-1.321-1.326H43.093c-.7-1.32-2.02-2.202-3.508-2.202h-.965V6.536C38.62 4.681 37.04 3 35.11 3h-3.34z',
                            clipRule: 'evenodd',
                        }),
                        H('path', {
                            fill: '#DC3545',
                            fillRule: 'evenodd',
                            d: 'M44.777 31.537c-.742 0-1.234.44-1.234 1.097s.492 1.098 1.234 1.098h19.47c.743 0 1.241-.441 1.241-1.098 0-.656-.582-1.097-1.24-1.097H44.777zM44.777 38.122c-.742 0-1.234.441-1.234 1.097 0 .657.492 1.098 1.234 1.098h19.47c.743 0 1.241-.441 1.241-1.098 0-.656-.582-1.097-1.24-1.097H44.777zM44.777 49.104c-.742 0-1.234.51-1.234 1.091 0 .657.492 1.098 1.234 1.098h19.47c.743 0 1.241-.441 1.241-1.098 0-.582-.582-1.09-1.24-1.09H44.777zM44.777 53.501c-.742 0-1.234.87-1.234 2.182 0 1.313.492 2.195 1.234 2.195h19.47c.743 0 1.241-.882 1.241-2.195 0-1.313-.582-2.182-1.24-2.182H44.777zM44.777 66.665c-.742 0-1.234.435-1.234 1.091 0 .582.492 1.098 1.234 1.098h19.47c.743 0 1.241-.516 1.241-1.098 0-.656-.582-1.09-1.24-1.09H44.777zM44.777 71.055c-.742 0-1.234.435-1.234 1.091 0 .663.492 1.098 1.234 1.098h19.47c.743 0 1.241-.435 1.241-1.098 0-.656-.582-1.09-1.24-1.09H44.777zM35.854 32.186v.61l-2.404 2.19-1.166-1.316a1.087 1.087 0 00-.92-.485c-.291 0-.592.11-.822.316-.494.435-.494 1.309-.16 1.919l1.986 2.19c.252.264.503.442.913.442.336 0 .586-.178.83-.354l1.828-1.574-.085 3.853H28.47v-7.79h7.383zm4.113-2.838a.99.99 0 00-.788.39l-1.163.962c-.334-.704-.913-1.139-1.66-1.139H27.81c-.997 0-1.827.876-1.827 2.015V40.5c0 1.14.83 2.013 1.827 2.013h8.547c.997 0 1.827-.873 1.827-2.013v-6.653l2.49-2.27c.662-.353.662-1.227.25-1.756a1.252 1.252 0 00-.956-.472zM35.854 49.736v.617l-2.404 2.21-1.166-1.327a1.1 1.1 0 00-.878-.409 1.33 1.33 0 00-.864.32c-.494.444-.494 1.329-.16 1.856l1.986 2.21c.252.356.503.446.913.446.336 0 .586-.09.83-.358l1.828-1.497-.085 3.71H28.47v-7.778h7.383zm4.148-2.827a1.1 1.1 0 00-.823.358l-1.163.97c-.334-.623-.913-1.238-1.66-1.238H27.81c-.997 0-1.827.883-1.827 2.033v9.009c0 1.15.83 2.032 1.827 2.032h8.547c.997 0 1.827-.882 1.827-2.032v-6.628l2.49-2.291c.662-.446.662-1.15.25-1.774a1.183 1.183 0 00-.92-.439zM35.854 66.707v.82l-2.404 1.867-1.166-1.122c-.215-.23-.542-.345-.878-.345-.313 0-.62.095-.864.27-.494.301-.494 1.122-.16 1.565l1.986 1.868c.252.301.503.37.913.37.336 0 .586-.069.83-.295l1.828-1.27-.085 2.837H28.47v-6.565h7.383zm-8.045-2.237c-.997 0-1.827.744-1.827 1.642v7.678c0 .898.83 1.642 1.827 1.642h8.547c.997 0 1.827-.744 1.827-1.642v-5.367l2.49-1.942c.579-.369.662-1.115.166-1.568-.173-.156-.487-.25-.821-.25-.294 0-.608.077-.839.25l-1.08.747c-.166-.67-.913-1.19-1.743-1.19H27.81z',
                            clipRule: 'evenodd',
                        }),
                    );
                },
                _ = c.a.createElement;
            var k = function () {
                    return _(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '96', height: '96', fill: 'none', viewBox: '0 0 96 96' },
                        _('path', { fill: '#fff', d: 'M0 0H96V96H0z' }),
                        _(
                            'g',
                            { fill: '#DC3545', fillRule: 'evenodd', clipPath: 'url(#clip0)', clipRule: 'evenodd' },
                            _('path', {
                                d: 'M80.677 27.687v31.194H65.68c-.816 0-1.362.603-1.362 1.34 0 .806.546 1.342 1.362 1.342h14.996v1.542H15.46v-1.542h44.453c.74 0 1.355-.536 1.355-1.341 0-.738-.615-1.341-1.355-1.341H40.978V27.687h39.699zm1.083 38.1l4.411 1.674H9.76l4.48-1.674h67.52zm-22.395 4.429l-1.288 1.273H38.197l-1.293-1.273h22.462zm32.849 0v.264h.067c0 2.417-2.034 4.429-4.48 4.429H8.335c-2.513 0-4.549-2.012-4.549-4.429v-.264h29.321l3.528 3.554c.268.266.61.4.945.4h21.108c.34 0 .677-.134.95-.4l3.53-3.554h29.046zM17.022 20.779c-2.445 0-4.412 1.944-4.412 4.361v17.037c0 .738.61 1.341 1.357 1.341.815 0 1.356-.603 1.356-1.34v-14.56h22.873v31.195H15.323V47.608c0-.738-.54-1.34-1.356-1.34-.747 0-1.357.602-1.357 1.34v15.83l-10.722 4.09a1.374 1.374 0 00-.883 1.28v1.605c0 3.893 3.255 7.11 7.194 7.11h79.535c3.937 0 7.192-3.217 7.192-7.11v-1.605a1.041 1.041 0 00-.747-1.144l-10.722-4.158V25.14c0-2.417-1.965-4.361-4.41-4.361H66.832c-.747 0-1.357.603-1.357 1.34 0 .739.61 1.342 1.357 1.342h12.215c.883 0 1.56.67 1.63 1.475H15.46c.07-.805.747-1.475 1.563-1.475h44.314c.747 0 1.357-.603 1.357-1.341s-.61-1.341-1.357-1.341H17.022z',
                            }),
                            _('path', {
                                d: 'M26.635 34.058v1.874H21.41v-1.874h5.225zm-6.594-2.643a1.3 1.3 0 00-1.305 1.29v4.515a1.3 1.3 0 001.305 1.29h7.9c.784 0 1.305-.58 1.305-1.29v-4.515c.131-.646-.521-1.29-1.305-1.29h-7.9zM20.132 40.287c-.767 0-1.39.352-1.39.885 0 .487.623.885 1.39.885h14.944c.835 0 1.39-.398 1.39-.885 0-.532-.555-.885-1.39-.885H20.132zM33.825 46.476v1.868h-5.287v-1.868h5.287zm-6.674-2.646c-.727 0-1.32.586-1.32 1.293v4.509c0 .707.593 1.287 1.32 1.287h7.994c.795 0 1.321-.58 1.321-1.287v-4.509c0-.707-.526-1.293-1.32-1.293H27.15zM20.132 52.698c-.767 0-1.39.397-1.39.885 0 .532.623.885 1.39.885h14.944c.835 0 1.39-.353 1.39-.885 0-.488-.555-.885-1.39-.885H20.132zM52.712 36.736c-.36 0-.725.135-1 .404l-5.954 5.754a1.289 1.289 0 00-.418.933c0 .338.14.668.418.94l5.954 5.753a1.374 1.374 0 001.937 0c.553-.466.553-1.337.07-1.938l-4.914-4.755 4.915-4.748a1.48 1.48 0 000-1.939c-.28-.27-.64-.404-1.008-.404zM68.13 36.736c-.37 0-.752.135-1.028.404-.48.467-.48 1.405 0 1.939l4.953 4.748-4.954 4.755a1.488 1.488 0 000 1.938c.277.27.625.404.965.404.346 0 .687-.134.963-.404l5.913-5.753a1.31 1.31 0 00.41-.94c.138-.332-.07-.669-.34-.933l-5.913-5.754c-.242-.27-.6-.404-.969-.404zM63.187 34.962c-.548 0-1.084.334-1.293.905l-5.636 15.011c-.272.673.069 1.481.824 1.747.208.068.348.068.548.068.556 0 1.104-.334 1.242-.876l5.7-15.012c.277-.672-.069-1.48-.894-1.747a1.34 1.34 0 00-.49-.096z',
                            }),
                        ),
                        _(
                            'defs',
                            null,
                            _('clipPath', { id: 'clip0' }, _('path', { fill: '#fff', d: 'M0 0H94V58.528H0z', transform: 'translate(1 19)' })),
                        ),
                    );
                },
                V = a.createElement,
                E = function () {
                    return V(
                        'div',
                        { className: 'p-2 space-y-8' },
                        V(
                            'h1',
                            { className: 'space-x-1 text-lg font-semibold tracking-wider md:text-center md:space-x-4 md:text-4xl md:uppercase ' },
                            V('span', null, 'My'),
                            V('span', { className: 'text-punch-500' }, 'Skills'),
                        ),
                        V(
                            'div',
                            { className: 'justify-center space-y-2 lg:space-x-16 lg:space-y-0 lg:flex' },
                            V(
                                'div',
                                { className: 'py-4 space-y-2 bg-white border shadow-2xl lg:w-80 border-nobel-500 md:p-8' },
                                V('div', { className: 'hidden md:grid place-items-center' }, V(N, null)),
                                V('h1', { className: 'pl-8 text-2xl font-semibold md:text-4xl md:text-center text-punch-500 md:pl-0' }, 'Design'),
                                V(
                                    'ul',
                                    { className: 'pl-8 list-disc md:text-lg md:font-medium' },
                                    V('li', null, 'Web Design'),
                                    V('li', null, 'Mobile App Design'),
                                    V('li', null, 'Desktop App Design'),
                                    V('li', null, 'Graphic/Print Design'),
                                ),
                            ),
                            V(
                                'div',
                                { className: 'py-4 space-y-2 bg-white border shadow-2xl lg:w-80 border-nobel-500 md:p-8' },
                                V('div', { className: 'hidden md:grid place-items-center' }, V(k, null)),
                                V(
                                    'h1',
                                    { className: 'pl-8 text-2xl font-semibold md:text-4xl md:text-center text-punch-500 md:pl-0' },
                                    'Development',
                                ),
                                V(
                                    'ul',
                                    { className: 'pl-8 list-disc md:text-lg md:font-medium' },
                                    V('li', null, 'Front-end Web App'),
                                    V('li', null, 'Mobile App'),
                                    V('li', null, 'Desktop App'),
                                    V('li', null, 'Data Structures'),
                                ),
                            ),
                            V(
                                'div',
                                { className: 'py-4 space-y-2 bg-white border shadow-2xl lg:w-80 border-nobel-500 md:p-8' },
                                V('div', { className: 'hidden md:grid place-items-center' }, V(C, null)),
                                V(
                                    'h1',
                                    { className: 'pl-8 text-2xl font-semibold md:text-4xl md:text-center text-punch-500 md:pl-0' },
                                    'Soft Skills',
                                ),
                                V(
                                    'ul',
                                    { className: 'pl-8 list-disc md:text-lg md:font-medium' },
                                    V('li', null, 'Teamwork Skills'),
                                    V('li', null, 'Content Creation'),
                                    V('li', null, 'Interpersonal Skills'),
                                    V('li', null, 'Public Speaking'),
                                ),
                            ),
                        ),
                    );
                },
                A = c.a.createElement;
            var T = function () {
                    return A(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '56', height: '56', fill: 'none', viewBox: '0 0 56 56' },
                        A('path', {
                            fill: '#000',
                            d: 'M28 0C12.536 0 0 12.536 0 28s12.536 28 28 28 28-12.536 28-28S43.464 0 28 0zm12.615 15.556l-12.625 9.98-13.036-9.98h25.66zm1.299 24.888h-28.26V19.771l13.027 9.88c.349.267.866.4 1.282.4.427 0 .797-.14 1.15-.418l12.8-10.183v20.994z',
                        }),
                    );
                },
                I = c.a.createElement;
            var S = function () {
                    return I(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '56', height: '56', fill: 'none', viewBox: '0 0 56 56' },
                        I('path', {
                            fill: '#000',
                            d: 'M28 0C12.538 0 0 12.538 0 28s12.538 28 28 28 28-12.538 28-28S43.462 0 28 0zm-8.137 42.328h-6.819V21.812h6.82v20.516zm-3.409-23.317h-.044c-2.289 0-3.769-1.575-3.769-3.544 0-2.013 1.526-3.545 3.858-3.545 2.333 0 3.769 1.532 3.813 3.545 0 1.969-1.48 3.544-3.858 3.544zm27.998 23.317h-6.818V31.353c0-2.759-.987-4.64-3.455-4.64-1.884 0-3.005 1.27-3.499 2.494-.18.438-.224 1.051-.224 1.664v11.457h-6.819s.09-18.591 0-20.516h6.82v2.905c.905-1.398 2.527-3.386 6.145-3.386 4.486 0 7.85 2.932 7.85 9.233v11.764z',
                        }),
                    );
                },
                P = c.a.createElement;
            var j = function () {
                    return P(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '56', height: '56', fill: 'none', viewBox: '0 0 56 56' },
                        P('path', {
                            fill: '#010002',
                            d: 'M28 .117c-15.464 0-28 12.536-28 28 0 13.87 10.095 25.355 23.33 27.58V33.957h-6.754v-7.822h6.754v-5.768c0-6.693 4.088-10.34 10.06-10.34 2.859 0 5.317.213 6.03.307v6.995l-4.141.002c-3.247 0-3.873 1.543-3.873 3.807v4.993h7.747l-1.01 7.822h-6.737v21.93C45.26 54.196 56 42.418 56 28.108 56 12.653 43.464.117 28 .117z',
                        }),
                    );
                },
                D = c.a.createElement;
            var L = function () {
                    return D(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '56', height: '56', fill: 'none', viewBox: '0 0 56 56' },
                        D(
                            'g',
                            null,
                            D('path', {
                                fill: '#000',
                                d: 'M47.796 8.198C36.859-2.735 19.13-2.732 8.198 8.204s-10.93 28.665.006 39.598 28.665 10.93 39.598-.006a28 28 0 00-.006-39.598zm-5.391 30.805a.066.066 0 01-.004.004v-.01l-1.419 1.41a7.467 7.467 0 01-7.046 2.016 24.81 24.81 0 01-7.206-3.23 33.292 33.292 0 01-5.684-4.582A33.544 33.544 0 0116.8 29.46 26.13 26.13 0 0113.608 23a7.466 7.466 0 011.866-7.625l1.662-1.661a1.185 1.185 0 011.676-.004l.004.004 5.245 5.245c.464.462.465 1.213.004 1.676l-.004.004-3.08 3.08a2.51 2.51 0 00-.261 3.267 36.025 36.025 0 003.686 4.302 35.793 35.793 0 004.854 4.08 2.53 2.53 0 003.229-.28l2.977-3.025a1.185 1.185 0 011.676-.004l.004.004 5.255 5.264c.464.462.466 1.212.004 1.676z',
                            }),
                        ),
                        D('defs', null, D('clipPath', null, D('path', { fill: '#fff', d: 'M0 0H56V56H0z' }))),
                    );
                },
                O = c.a.createElement;
            var R = function () {
                    return O(
                        'svg',
                        { xmlns: 'http://www.w3.org/2000/svg', width: '56', height: '56', fill: 'none', viewBox: '0 0 56 56' },
                        O('path', {
                            fill: '#000',
                            d: 'M28 12.25c-6.152 0-9.825 3.846-9.825 10.287 0 3.665 4.87 12.133 9.823 18.894 4.924-6.784 9.827-15.147 9.827-18.894 0-6.441-3.673-10.287-9.825-10.287zm-.01 14.694a4.315 4.315 0 110-8.63 4.315 4.315 0 010 8.63z',
                        }),
                        O('path', {
                            fill: '#000',
                            d: 'M28 0C12.536 0 0 12.536 0 28s12.536 28 28 28 28-12.536 28-28S43.464 0 28 0zm1.64 45.097L28 47.25l-1.63-2.153c-1.2-1.568-11.695-15.58-11.695-22.56C14.675 14.162 19.902 8.75 28 8.75c8.098 0 13.325 5.412 13.325 13.787 0 6.97-10.496 20.982-11.685 22.56z',
                        }),
                    );
                },
                B = a.createElement,
                q = function () {
                    return B(
                        'div',
                        { className: 'flex flex-col items-center justify-center p-4 space-y-2 bg-sandy-brown-500' },
                        B(
                            'div',
                            { className: 'flex space-x-4' },
                            B('a', { href: 'https://www.facebook.com/ltnhu.shelling', target: '__blank' }, B(j, null)),
                            B('a', { href: 'mailto: tamnhu.dahlia@gmail.com' }, B(T, null)),
                            B('a', { href: 'tel:+84901951454' }, B(L, null)),
                            B('a', { href: 'https://goo.gl/maps/VD5Lfb5VQwJwkFLf7', target: '__blank' }, B(R, null)),
                            B('a', { href: 'https://www.linkedin.com/in/shelling-dahlia/', target: '__blank' }, B(S, null)),
                        ),
                        B('p', { className: 'font-medium' }, '\xa9Copyright by Shelling'),
                    );
                },
                U = a.createElement,
                F = function () {
                    return U(
                        'div',
                        { className: 'px-2 py-8 bg-macaroni-and-cheese-500' },
                        U(
                            'form',
                            { className: 'md:w-2/5 mx-auto space-y-2' },
                            U('h1', { className: 'text-2xl text-center text-yellow-800 font ' }, 'GET IN TOUCH'),
                            U(
                                'div',
                                { className: 'justify-between space-y-2 md:space-x-4 md:flex md:space-y-0' },
                                U(
                                    'div',
                                    { className: 'md:flex-1' },
                                    U('input', { placeholder: 'Your name *', className: 'w-full p-2 focus:outline-none' }),
                                ),
                                U(
                                    'div',
                                    { className: 'md:flex-1' },
                                    U('input', { placeholder: 'Your email *', className: 'w-full p-2 focus:outline-none' }),
                                ),
                            ),
                            U(
                                'div',
                                null,
                                U('input', {
                                    placeholder: 'Subject',
                                    className:
                                        'w-full p-2 placeholder-black border border-black border-dashed focus:outline-none bg-macaroni-and-cheese-500',
                                }),
                            ),
                            U(
                                'div',
                                null,
                                U('textarea', { placeholder: 'Your Message *', name: '', id: '', className: 'w-full h-64 p-2 focus:outline-none' }),
                            ),
                            U('button', { className: 'p-2 text-black border border-black border-dashed focus:outline-none' }, 'SEND MESSAGE'),
                        ),
                    );
                },
                K = a.createElement;
            t.default = function () {
                return K(a.Fragment, null, K(x.a, y.home.header), K(u, null), K(w, null), K(M, null), K(E, null), K(F, null), K(q, null));
            };
        },
        '48fX': function (e, t, n) {
            var a = n('qhzo');
            e.exports = function (e, t) {
                if ('function' !== typeof t && null !== t) throw new TypeError('Super expression must either be null or a function');
                (e.prototype = Object.create(t && t.prototype, { constructor: { value: e, writable: !0, configurable: !0 } })), t && a(e, t);
            };
        },
        '5fIB': function (e, t, n) {
            var a = n('7eYB');
            e.exports = function (e) {
                if (Array.isArray(e)) return a(e);
            };
        },
        '8Kt/': function (e, t, n) {
            'use strict';
            n('oI91');
            (t.__esModule = !0), (t.defaultHead = u), (t.default = void 0);
            var a,
                c = (function (e) {
                    if (e && e.__esModule) return e;
                    if (null === e || ('object' !== typeof e && 'function' !== typeof e)) return { default: e };
                    var t = s();
                    if (t && t.has(e)) return t.get(e);
                    var n = {},
                        a = Object.defineProperty && Object.getOwnPropertyDescriptor;
                    for (var c in e)
                        if (Object.prototype.hasOwnProperty.call(e, c)) {
                            var l = a ? Object.getOwnPropertyDescriptor(e, c) : null;
                            l && (l.get || l.set) ? Object.defineProperty(n, c, l) : (n[c] = e[c]);
                        }
                    (n.default = e), t && t.set(e, n);
                    return n;
                })(n('q1tI')),
                l = (a = n('Xuae')) && a.__esModule ? a : { default: a },
                r = n('lwAK'),
                i = n('FYa8'),
                o = n('/0+H');
            function s() {
                if ('function' !== typeof WeakMap) return null;
                var e = new WeakMap();
                return (
                    (s = function () {
                        return e;
                    }),
                    e
                );
            }
            function u() {
                var e = arguments.length > 0 && void 0 !== arguments[0] && arguments[0],
                    t = [c.default.createElement('meta', { charSet: 'utf-8' })];
                return e || t.push(c.default.createElement('meta', { name: 'viewport', content: 'width=device-width' })), t;
            }
            function h(e, t) {
                return 'string' === typeof t || 'number' === typeof t
                    ? e
                    : t.type === c.default.Fragment
                    ? e.concat(
                          c.default.Children.toArray(t.props.children).reduce(function (e, t) {
                              return 'string' === typeof t || 'number' === typeof t ? e : e.concat(t);
                          }, []),
                      )
                    : e.concat(t);
            }
            var d = ['name', 'httpEquiv', 'charSet', 'itemProp'];
            function m(e, t) {
                return e
                    .reduce(function (e, t) {
                        var n = c.default.Children.toArray(t.props.children);
                        return e.concat(n);
                    }, [])
                    .reduce(h, [])
                    .reverse()
                    .concat(u(t.inAmpMode))
                    .filter(
                        (function () {
                            var e = new Set(),
                                t = new Set(),
                                n = new Set(),
                                a = {};
                            return function (c) {
                                var l = !0,
                                    r = !1;
                                if (c.key && 'number' !== typeof c.key && c.key.indexOf('$') > 0) {
                                    r = !0;
                                    var i = c.key.slice(c.key.indexOf('$') + 1);
                                    e.has(i) ? (l = !1) : e.add(i);
                                }
                                switch (c.type) {
                                    case 'title':
                                    case 'base':
                                        t.has(c.type) ? (l = !1) : t.add(c.type);
                                        break;
                                    case 'meta':
                                        for (var o = 0, s = d.length; o < s; o++) {
                                            var u = d[o];
                                            if (c.props.hasOwnProperty(u))
                                                if ('charSet' === u) n.has(u) ? (l = !1) : n.add(u);
                                                else {
                                                    var h = c.props[u],
                                                        m = a[u] || new Set();
                                                    ('name' === u && r) || !m.has(h) ? (m.add(h), (a[u] = m)) : (l = !1);
                                                }
                                        }
                                }
                                return l;
                            };
                        })(),
                    )
                    .reverse()
                    .map(function (e, t) {
                        var n = e.key || t;
                        return c.default.cloneElement(e, { key: n });
                    });
            }
            function f(e) {
                var t = e.children,
                    n = (0, c.useContext)(r.AmpStateContext),
                    a = (0, c.useContext)(i.HeadManagerContext);
                return c.default.createElement(l.default, { reduceComponentsToState: m, headManager: a, inAmpMode: (0, o.isInAmpMode)(n) }, t);
            }
            f.rewind = function () {};
            var p = f;
            t.default = p;
        },
        '8oxB': function (e, t) {
            var n,
                a,
                c = (e.exports = {});
            function l() {
                throw new Error('setTimeout has not been defined');
            }
            function r() {
                throw new Error('clearTimeout has not been defined');
            }
            function i(e) {
                if (n === setTimeout) return setTimeout(e, 0);
                if ((n === l || !n) && setTimeout) return (n = setTimeout), setTimeout(e, 0);
                try {
                    return n(e, 0);
                } catch (t) {
                    try {
                        return n.call(null, e, 0);
                    } catch (t) {
                        return n.call(this, e, 0);
                    }
                }
            }
            !(function () {
                try {
                    n = 'function' === typeof setTimeout ? setTimeout : l;
                } catch (e) {
                    n = l;
                }
                try {
                    a = 'function' === typeof clearTimeout ? clearTimeout : r;
                } catch (e) {
                    a = r;
                }
            })();
            var o,
                s = [],
                u = !1,
                h = -1;
            function d() {
                u && o && ((u = !1), o.length ? (s = o.concat(s)) : (h = -1), s.length && m());
            }
            function m() {
                if (!u) {
                    var e = i(d);
                    u = !0;
                    for (var t = s.length; t; ) {
                        for (o = s, s = []; ++h < t; ) o && o[h].run();
                        (h = -1), (t = s.length);
                    }
                    (o = null),
                        (u = !1),
                        (function (e) {
                            if (a === clearTimeout) return clearTimeout(e);
                            if ((a === r || !a) && clearTimeout) return (a = clearTimeout), clearTimeout(e);
                            try {
                                a(e);
                            } catch (t) {
                                try {
                                    return a.call(null, e);
                                } catch (t) {
                                    return a.call(this, e);
                                }
                            }
                        })(e);
                }
            }
            function f(e, t) {
                (this.fun = e), (this.array = t);
            }
            function p() {}
            (c.nextTick = function (e) {
                var t = new Array(arguments.length - 1);
                if (arguments.length > 1) for (var n = 1; n < arguments.length; n++) t[n - 1] = arguments[n];
                s.push(new f(e, t)), 1 !== s.length || u || i(m);
            }),
                (f.prototype.run = function () {
                    this.fun.apply(null, this.array);
                }),
                (c.title = 'browser'),
                (c.browser = !0),
                (c.env = {}),
                (c.argv = []),
                (c.version = ''),
                (c.versions = {}),
                (c.on = p),
                (c.addListener = p),
                (c.once = p),
                (c.off = p),
                (c.removeListener = p),
                (c.removeAllListeners = p),
                (c.emit = p),
                (c.prependListener = p),
                (c.prependOnceListener = p),
                (c.listeners = function (e) {
                    return [];
                }),
                (c.binding = function (e) {
                    throw new Error('process.binding is not supported');
                }),
                (c.cwd = function () {
                    return '/';
                }),
                (c.chdir = function (e) {
                    throw new Error('process.chdir is not supported');
                }),
                (c.umask = function () {
                    return 0;
                });
        },
        FYa8: function (e, t, n) {
            'use strict';
            var a;
            (t.__esModule = !0), (t.HeadManagerContext = void 0);
            var c = ((a = n('q1tI')) && a.__esModule ? a : { default: a }).default.createContext({});
            t.HeadManagerContext = c;
        },
        T0f4: function (e, t) {
            function n(t) {
                return (
                    (e.exports = n =
                        Object.setPrototypeOf
                            ? Object.getPrototypeOf
                            : function (e) {
                                  return e.__proto__ || Object.getPrototypeOf(e);
                              }),
                    n(t)
                );
            }
            e.exports = n;
        },
        UkVL: function (e, t, n) {
            'use strict';
            n.d(t, 'a', function () {
                return a;
            });
            var a = function (e) {
                return e
                    .split(' ')
                    .map(function (e) {
                        return e.charAt(0).toUpperCase() + e.slice(1);
                    })
                    .join(' ');
            };
        },
        Xuae: function (e, t, n) {
            'use strict';
            var a = n('mPvQ'),
                c = n('/GRZ'),
                l = n('i2R6'),
                r = (n('qXWd'), n('48fX')),
                i = n('tCBg'),
                o = n('T0f4');
            function s(e) {
                var t = (function () {
                    if ('undefined' === typeof Reflect || !Reflect.construct) return !1;
                    if (Reflect.construct.sham) return !1;
                    if ('function' === typeof Proxy) return !0;
                    try {
                        return Date.prototype.toString.call(Reflect.construct(Date, [], function () {})), !0;
                    } catch (e) {
                        return !1;
                    }
                })();
                return function () {
                    var n,
                        a = o(e);
                    if (t) {
                        var c = o(this).constructor;
                        n = Reflect.construct(a, arguments, c);
                    } else n = a.apply(this, arguments);
                    return i(this, n);
                };
            }
            (t.__esModule = !0), (t.default = void 0);
            var u = n('q1tI'),
                h = (function (e) {
                    r(n, e);
                    var t = s(n);
                    function n(e) {
                        var l;
                        return (
                            c(this, n),
                            ((l = t.call(this, e))._hasHeadManager = void 0),
                            (l.emitChange = function () {
                                l._hasHeadManager &&
                                    l.props.headManager.updateHead(l.props.reduceComponentsToState(a(l.props.headManager.mountedInstances), l.props));
                            }),
                            (l._hasHeadManager = l.props.headManager && l.props.headManager.mountedInstances),
                            l
                        );
                    }
                    return (
                        l(n, [
                            {
                                key: 'componentDidMount',
                                value: function () {
                                    this._hasHeadManager && this.props.headManager.mountedInstances.add(this), this.emitChange();
                                },
                            },
                            {
                                key: 'componentDidUpdate',
                                value: function () {
                                    this.emitChange();
                                },
                            },
                            {
                                key: 'componentWillUnmount',
                                value: function () {
                                    this._hasHeadManager && this.props.headManager.mountedInstances.delete(this), this.emitChange();
                                },
                            },
                            {
                                key: 'render',
                                value: function () {
                                    return null;
                                },
                            },
                        ]),
                        n
                    );
                })(u.Component);
            t.default = h;
        },
        YFqc: function (e, t, n) {
            e.exports = n('cTJO');
        },
        Yh1s: function (e, t, n) {
            'use strict';
            (function (e) {
                var a = n('q1tI'),
                    c = n('g4pe'),
                    l = n.n(c),
                    r = n('UkVL'),
                    i = a.createElement;
                t.a = function (t) {
                    var n = t.title,
                        a = void 0 === n ? '' : n,
                        c = t.isIndexPage,
                        o = void 0 !== c && c,
                        s = t.isFollowPage,
                        u = void 0 === s || s,
                        h = t.description,
                        d = void 0 === h ? '' : h,
                        m = t.canonical,
                        f = void 0 === m ? '' : m,
                        p = t.imageUrl,
                        v = void 0 === p ? '/images/image.png' : p,
                        g = u ? 'follow' : 'nofollow',
                        w = ''.concat(o ? 'index' : 'noindex', ',').concat(g),
                        x = ''.concat(a);
                    return i(
                        l.a,
                        null,
                        i('meta', { charSet: 'UTF-8' }),
                        i('meta', { httpEquiv: 'Content-Type', content: 'text/html;charset=UTF-8' }),
                        i('meta', { name: 'viewport', content: 'width=device-width, initial-scale=1.0, user-scalable=0' }),
                        i('title', null, Object(r.a)(x)),
                        i('meta', { name: 'description', content: d }),
                        i('meta', { name: 'robots', content: w }),
                        i('meta', { name: 'keywords', content: 'Lam Tam Nhu, portfolio, personal project' }),
                        i('link', { href: 'https://shelling.site' + f, rel: 'canonical' }),
                        i('meta', { property: 'og:type', content: 'article' }),
                        i('meta', { property: 'og:title', content: x }),
                        i('meta', { property: 'og:description', content: d }),
                        i('meta', { property: 'og:image', content: v }),
                        i('meta', { property: 'fb:app_id', content: e.env.FB_APP_ID }),
                        i('meta', { name: 'twitter:title', content: x }),
                        i('meta', { name: 'twitter:description', content: d }),
                        i('meta', { name: 'twitter:image', content: v }),
                        i('meta', { name: 'twitter:card', content: 'summary_large_image' }),
                    );
                };
            }.call(this, n('8oxB')));
        },
        cTJO: function (e, t, n) {
            'use strict';
            var a = n('zoAU'),
                c = n('7KCV');
            (t.__esModule = !0), (t.default = void 0);
            var l = c(n('q1tI')),
                r = n('elyg'),
                i = n('nOHt'),
                o = n('vNVm'),
                s = {};
            function u(e, t, n, a) {
                if (e && (0, r.isLocalURL)(t)) {
                    e.prefetch(t, n, a).catch(function (e) {
                        0;
                    });
                    var c = a && 'undefined' !== typeof a.locale ? a.locale : e && e.locale;
                    s[t + '%' + n + (c ? '%' + c : '')] = !0;
                }
            }
            var h = function (e) {
                var t = !1 !== e.prefetch,
                    n = (0, i.useRouter)(),
                    c = (n && n.pathname) || '/',
                    h = l.default.useMemo(
                        function () {
                            var t = (0, r.resolveHref)(c, e.href, !0),
                                n = a(t, 2),
                                l = n[0],
                                i = n[1];
                            return { href: l, as: e.as ? (0, r.resolveHref)(c, e.as) : i || l };
                        },
                        [c, e.href, e.as],
                    ),
                    d = h.href,
                    m = h.as,
                    f = e.children,
                    p = e.replace,
                    v = e.shallow,
                    g = e.scroll,
                    w = e.locale;
                'string' === typeof f && (f = l.default.createElement('a', null, f));
                var x = l.Children.only(f),
                    y = x && 'object' === typeof x && x.ref,
                    b = (0, o.useIntersection)({ rootMargin: '200px' }),
                    M = a(b, 2),
                    z = M[0],
                    N = M[1],
                    H = l.default.useCallback(
                        function (e) {
                            z(e), y && ('function' === typeof y ? y(e) : 'object' === typeof y && (y.current = e));
                        },
                        [y, z],
                    );
                (0, l.useEffect)(
                    function () {
                        var e = N && t && (0, r.isLocalURL)(d),
                            a = 'undefined' !== typeof w ? w : n && n.locale,
                            c = s[d + '%' + m + (a ? '%' + a : '')];
                        e && !c && u(n, d, m, { locale: a });
                    },
                    [m, d, N, w, t, n],
                );
                var C = {
                    ref: H,
                    onClick: function (e) {
                        x.props && 'function' === typeof x.props.onClick && x.props.onClick(e),
                            e.defaultPrevented ||
                                (function (e, t, n, a, c, l, i, o) {
                                    ('A' !== e.currentTarget.nodeName ||
                                        (!(function (e) {
                                            var t = e.currentTarget.target;
                                            return (
                                                (t && '_self' !== t) ||
                                                e.metaKey ||
                                                e.ctrlKey ||
                                                e.shiftKey ||
                                                e.altKey ||
                                                (e.nativeEvent && 2 === e.nativeEvent.which)
                                            );
                                        })(e) &&
                                            (0, r.isLocalURL)(n))) &&
                                        (e.preventDefault(),
                                        null == i && (i = a.indexOf('#') < 0),
                                        t[c ? 'replace' : 'push'](n, a, { shallow: l, locale: o, scroll: i }));
                                })(e, n, d, m, p, v, g, w);
                    },
                    onMouseEnter: function (e) {
                        (0, r.isLocalURL)(d) &&
                            (x.props && 'function' === typeof x.props.onMouseEnter && x.props.onMouseEnter(e), u(n, d, m, { priority: !0 }));
                    },
                };
                if (e.passHref || ('a' === x.type && !('href' in x.props))) {
                    var _ = 'undefined' !== typeof w ? w : n && n.locale,
                        k = n && n.isLocaleDomain && (0, r.getDomainLocale)(m, _, n && n.locales, n && n.domainLocales);
                    C.href = k || (0, r.addBasePath)((0, r.addLocale)(m, _, n && n.defaultLocale));
                }
                return l.default.cloneElement(x, C);
            };
            t.default = h;
        },
        g4pe: function (e, t, n) {
            e.exports = n('8Kt/');
        },
        kG2m: function (e, t) {
            e.exports = function () {
                throw new TypeError(
                    'Invalid attempt to spread non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.',
                );
            };
        },
        lwAK: function (e, t, n) {
            'use strict';
            var a;
            (t.__esModule = !0), (t.AmpStateContext = void 0);
            var c = ((a = n('q1tI')) && a.__esModule ? a : { default: a }).default.createContext({});
            t.AmpStateContext = c;
        },
        mPvQ: function (e, t, n) {
            var a = n('5fIB'),
                c = n('rlHP'),
                l = n('KckH'),
                r = n('kG2m');
            e.exports = function (e) {
                return a(e) || c(e) || l(e) || r();
            };
        },
        oI91: function (e, t) {
            e.exports = function (e, t, n) {
                return t in e ? Object.defineProperty(e, t, { value: n, enumerable: !0, configurable: !0, writable: !0 }) : (e[t] = n), e;
            };
        },
        qXWd: function (e, t) {
            e.exports = function (e) {
                if (void 0 === e) throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
                return e;
            };
        },
        rlHP: function (e, t) {
            e.exports = function (e) {
                if ('undefined' !== typeof Symbol && Symbol.iterator in Object(e)) return Array.from(e);
            };
        },
        tCBg: function (e, t, n) {
            var a = n('C+bE'),
                c = n('qXWd');
            e.exports = function (e, t) {
                return !t || ('object' !== a(t) && 'function' !== typeof t) ? c(e) : t;
            };
        },
        vNVm: function (e, t, n) {
            'use strict';
            var a = n('zoAU');
            (t.__esModule = !0),
                (t.useIntersection = function (e) {
                    var t = e.rootMargin,
                        n = e.disabled || !r,
                        o = (0, c.useRef)(),
                        s = (0, c.useState)(!1),
                        u = a(s, 2),
                        h = u[0],
                        d = u[1],
                        m = (0, c.useCallback)(
                            function (e) {
                                o.current && (o.current(), (o.current = void 0)),
                                    n ||
                                        h ||
                                        (e &&
                                            e.tagName &&
                                            (o.current = (function (e, t, n) {
                                                var a = (function (e) {
                                                        var t = e.rootMargin || '',
                                                            n = i.get(t);
                                                        if (n) return n;
                                                        var a = new Map(),
                                                            c = new IntersectionObserver(function (e) {
                                                                e.forEach(function (e) {
                                                                    var t = a.get(e.target),
                                                                        n = e.isIntersecting || e.intersectionRatio > 0;
                                                                    t && n && t(n);
                                                                });
                                                            }, e);
                                                        return i.set(t, (n = { id: t, observer: c, elements: a })), n;
                                                    })(n),
                                                    c = a.id,
                                                    l = a.observer,
                                                    r = a.elements;
                                                return (
                                                    r.set(e, t),
                                                    l.observe(e),
                                                    function () {
                                                        r.delete(e), l.unobserve(e), 0 === r.size && (l.disconnect(), i.delete(c));
                                                    }
                                                );
                                            })(
                                                e,
                                                function (e) {
                                                    return e && d(e);
                                                },
                                                { rootMargin: t },
                                            )));
                            },
                            [n, t, h],
                        );
                    return (
                        (0, c.useEffect)(
                            function () {
                                if (!r && !h) {
                                    var e = (0, l.requestIdleCallback)(function () {
                                        return d(!0);
                                    });
                                    return function () {
                                        return (0, l.cancelIdleCallback)(e);
                                    };
                                }
                            },
                            [h],
                        ),
                        [m, h]
                    );
                });
            var c = n('q1tI'),
                l = n('0G5g'),
                r = 'undefined' !== typeof IntersectionObserver;
            var i = new Map();
        },
    },
    [['/EDR', 0, 1, 2]],
]);
